...
p
ggMultisave("DT/img/ex/tc_finale", 
            plot = p,
            width = 8, height = 3)
...
all_est <- c(lp_est, list("LC param. locale" = loc_lc_est),
              list("LC param. locale" = loc_ql_est), 
              rkhs_est,
              list(FST = fst_est))
all_if <- c(lp_if, list("LC param. locale" = loc_lc_if),
            list("LC param. locale" = loc_ql_if), 
            rkhs_if,
            list(FST = fst_if))
all_plots_est <- lapply(
  names(all_est), 
  function(x) plot_est(all_est[[x]]) + ggtitle(latex2exp::TeX(x))
)
all_plots_prev <- lapply(
  names(all_if), 
  function(x) plot_prevs(all_if[[x]]) + ggtitle(latex2exp::TeX(x))
)
