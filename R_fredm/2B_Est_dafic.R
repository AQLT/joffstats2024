...
if(!dir.exists("results_fredm/localic_daf"))
  dir.create("results_fredm/localic_daf")
library(rjd3filters)
library(AQLThesis)
library(future)
plan(multisession)
...
for (method in c("LC","QL")){
  ...
  for(s in list.files("data_fredm/byseries", full.names = TRUE)){
    for(d in 2:3){
      ...
      nom_f_s <- sprintf("results_fredm/localic_daf/%s_%s%s.RDS",
                         name_file, tolower(method), complement)
      nom_f_s_tp <- 
        sprintf("results_fredm/localic_daf/%s_%s%s_tp.RDS",
                name_file,
