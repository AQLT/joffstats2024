  ...
  for(s in list.files("data_fredm/byseries", full.names = TRUE)){
    for(d in 2:3){
      for(h in 3:6) {
        ...
        nom_f_s <- sprintf("results_fredm/localic_daf_trunc/%s_%s_h%i%s.RDS",
                           name_file, tolower(method), h, complement)
        nom_f_s_tp <- 
          sprintf("results_fredm/localic_daf_trunc/%s_%s_h%i%s_tp.RDS",
                  name_file,
                  tolower(method), h, complement)
        
        data_info <- readRDS(sprintf("data_fredm/byseriespente_daf_nber/%s_h%i.RDS",
