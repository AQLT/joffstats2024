...
if(!dir.exists("results_simul/localic_daf_trunc_nn"))
  dir.create("results_simul/localic_daf_trunc_nn")
library(rjd3filters)
library(AQLThesis)
library(future)
plan(multisession)
...
h_filter <- lp_filter(horizon = h, kernel = kernel)@sfilter
...
for (method in c("LC","QL")){
  ...
  for(s in list.files("data_simul/byseries", full.names = TRUE)){
    for(d in 2:3){
      ...
      nom_f_s <- sprintf("results_simul/localic_daf_trunc_nn/%s_%s%s.RDS",
                         name_file, tolower(method), complement)
      nom_f_s_tp <- 
        sprintf("results_simul/localic_daf_trunc_nn/%s_%s%s_tp.RDS",
                name_file,
                tolower(method), complement)
      
      nom_f_s_rev_fe <- sprintf("results_simul/localic_daf_trunc_nn/%s_%s%s_fe_rev.RDS",
                                name_file,
                                tolower(method), complement)
      nom_f_s_rev_ce <- sprintf("results_simul/localic_daf_trunc_nn/%s_%s%s_ce_rev.RDS",
                                name_file,
                                tolower(method), complement)
