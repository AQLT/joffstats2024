...
It enables to reproduce Henderson's symmetric and Musgrave's asymmetric filters used in the X-13ARIMA-SEATS seasonal adjustment algorithm.
...
Thus, even though trend-cycle extraction methods are generally applied to seasonally adjusted series, the estimation of these series also depends on trend-cycle estimation methods.
...
In particular, this class encompass Henderson's symmetric filter and Musgrave's asymmetric filters.
...
Those extensions are implemented in the statistical software R [@logicielr] throw the `rjd3filters` package [@rjd3filters] and can therefore be easily used.
...
\justifying
*Lecture: on monthly times series, 12-month cycles (associated to the frequency $2\pi/12=\pi/6$) are amplified by 10% ($G_{\boldsymbol\theta}(\pi/6)\simeq1.1$) and associated turning points are detected with a delay of 1 months ($\varphi_{\boldsymbol\theta}(\pi/6)/(\pi/6)\simeq -1$).*
...
For the penalty function, the authors suggest $f\colon(\rho,\varphi)\mapsto\rho^2\sin(\varphi)^2$ and show than in that case the criteria can be written in a quadratic form:
$$
...
\justifying
*Lecture: the dotted vertical line corresponds to the date of the turning point (February 2001).*
...
In the section we discuss the applicability of the presented methods to other frequencies (@sec-other-freq) and some limits of the presented methods (for example sensibility to outliers like COVID-19, @sec-out-covid) .

### Other frequencies {#sec-other-freq}

In this paper, we have focused on monthly series, but the methods presented can be applied to other frequencies.
...
Mechanically, since fewer points are used, there will be less differences between methods.
...
The length of the filters can be determined using local polynomial methods [see for example @Loader1999 for a description of bandwidth selection approaches] but it will surely be necessary to use higher bandwidth than for monthly series.
...
Moreover, since the bias is higher, the local parametrisation of the filters will be more important.
...
Furthermore, as shown in particular by @dagum1996new, the final symmetric filter used by X-13ARIMA-SEATS to extract the trend-cycle (and therefore the one indirectly used when applying the methods to seasonally-adjusted series), cycles of length 9 and 10 months (generally associated with noise rather than the trend-cycle) are reduced by just 38%.
The final asymmetric filters even amplify 9 or 10-month cycles.
...
[@Fig-ce16ovcovid; @fig-ce16ov-previmp-covid] show the successive estimates of the trend-cycle and the associated implicit forecast for the US employement during the COVID-19.
...
For the LC method, local parametrisation has few impacts on the estimates of the trend-cycle but the associated implicit forecasts are not plausible: the final estimate of the slope is biased by the outlier.
...
\justifying
*Lecture: the dotted vertical line corresponds to the date of the turning point (April 2020) but the detected one with the 13-term Henderson filter is June 2020.*
...
\justifying
*Lecture: the curve "Apr 2020" corresponds to the implicit forecasts associated to the asymmetric filters used for the trend-cycle estimations using the data observed until April 2001.*
...
- Adjustment of outliers prior to filtering, for example with a RegARIMA model or other correction modules like the one.

- Increase the bandwidth of the filter used to extract the trend-cycle.

- Since all the filter studied in this paper are equivalent to a local polynomial model, an external regressor can be added to the linear model to handle the outlier (the matrix $\boldsymbol X$ would then be modified).
