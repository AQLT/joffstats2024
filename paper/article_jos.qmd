...
For example, the X-11 decomposition method (used in X-13ARIMA-SEATS) uses several moving averages to estimate the main components of a time series. 
...
The extensions have been implemented in the R `rjd3filters` package (available at <https://github.com/rjdemetra/rjd3filters>) and can therefore be easily used.
...
$$
\forall t \::\:TC_t = \mu(x_0) + \mu'(x_0)(x-x_0)+\dots +
...
$$ {#eq-lpgeneralmodel}
where $[\boldsymbol U,\boldsymbol Z]$ is of full rank and forms a subset of the columns of $X$ and the variance of the error is defined as $\boldsymbol D = \sigma^2\boldsymbol K^{-1}$ to directly take into account the weights associated to the observations.
...
- The variance $\sigma^2$ can be estimated using the observed data set and the symmetric filter $(w_{-h},\dots,w_h)$.
...
$$
\hat\sigma^2=\frac{1}{(n-2h)-2\nu_1+\nu_2}\sum_{t=h+1}^{n-h}(y_t-\hat \mu_t)^2.
$$
...
In the case of local polynomial smoothings, they can be linked to the coefficient of the associated filter: $\nu_1 = (n-h)w_0$ (number of observations multiplied by the central coefficient of the moving average) and $\nu_2 = (n-h)\sum_{i=-h}^{h} w_i^2$ (number of observations multiplied by the sum of squares of coefficients of the moving average).
Thus, the variance can be estimated by:
$$
\hat\sigma^2=\frac{1}{n-2h}\sum_{t=h+1}^{n-h}\frac{(y_t-\hat \mu_t)^2}{1-2w_0^2+\sum_{i=-h}^{h} w_i^2}.
$$
