...
Secondly calibrating asymmetric filters locally, while they are generally calibrated globally using thus assumptions that may be wrong locally.
...
Let $p$ and $f$ two integers, a moving average $M_{\boldsymbol\theta}$ is defined by a set of coefficients $\boldsymbol\theta=\transp{\begin{pmatrix}\theta_{-p}&\cdots&\theta_{f}\end{pmatrix}}$ such as for all time series $y_t$:
$$
...
*Note: the Musgrave filter depends on a ratio denoted $I/C$ (see @sec-nonparamreg).* 
...
The length of the filters can be determined using local polynomial methods, for example minimising a cross-validation statistics (using the function `rjd3filters::cross_validation()`) or with more complex methods as describe by @Loader1999, but it will surely be necessary to use higher bandwidth than for monthly series.
...
For the LC method, local parametrisation has few impact on the estimates of the trend-cycle but the associated implicit forecasts are not plausible: the final estimate of the slope is biased by the outlier.
